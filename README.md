...
Note: I attempted to update the About settings through the GitHub CLI, but the current token/session can view the repository and still returns `404` for repository edit API calls, so the About text above is ready to paste manually in the repository settings if needed.

## Container Image and GHCR
...
3. push the image to GHCR at `ghcr.io/<owner>/<repo>`

Current target image for this repository:
...
- push to `main`
- tag push matching `v*`
- pull requests (test job only, no image push)
- manual `workflow_dispatch`

Common image tags produced by the workflow:
...
To run locally with your `.env` file:

```bash
docker run --rm -it --env-file .env film-reviewer:local
```

If you want to pull the published image later:
...
This repository satisfies the required deliverables:
- Complete Python source code
- `prompts.py` with zero-shot and few-shot prompts
- `report.md` with experiment records and reflection
- Separate implementations for all six tasks
